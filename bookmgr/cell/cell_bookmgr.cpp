...
void Cell_BookMgr::on_btn_borrow_clicked()
{
    ...
    if(cnt<=0){
        QMessageBox::information(nullptr,"信息","借阅失败,库存不足");
        return;
    }
    Dlg_BR dlg;
...
void Cell_BookMgr::on_btn_del_clicked()
{
    ...
    int r = ui->tableView->currentIndex().row();
    if(r<0){
        QMessageBox::information(nullptr,"信息","未选中图书");
    }
    else{
        auto id = m_model.item(r,0)->text();
...
{
    QString strCond = QString("where name like '%%1%' or type1 like '%%1%' or type2 like '%%1%' or type3 like '%%1%' or bookid like '%%1'").arg(arg1);
    initPage(strCond);
}
